...
resource "azurerm_application_gateway" "network" {
  ...
  identity {
    type = "UserAssigned"
    identity_ids  = [azurerm_user_assigned_identity.agic_identity[count.index].id]
  }
...
resource "azurerm_role_assignment" "agw_keyvault_admin" {
  count                = var.resource_count
  scope                = azurerm_key_vault.example[count.index].id
  role_definition_name = "Key Vault Administrator"
  principal_id         = azurerm_user_assigned_identity.agic_identity[count.index].principal_id
}